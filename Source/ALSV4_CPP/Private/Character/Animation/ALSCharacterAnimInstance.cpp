...
void UALSCharacterAnimInstance::NativeBeginPlay()
{
	...
	CombatComponent = Character->FindComponentByClass<UCPC_Combat>();
}
...
void UALSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (MovementState.Grounded())
	{
		...
		if (Grounded.bShouldMove)
		{
			...
			// Additional logic for rotating towards the enemy
			//if (CanRotateToEnemy())
			//{
			//	//GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Red, TEXT("canrotate"));
			//	//RotateToEnemyCheck(DeltaSeconds);
		...
		else
		{
			...
			//	//GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Red, TEXT("Stand"));
...
bool UALSCharacterAnimInstance::CanTurnToEnemy() const
{
	FVector DirectionToEnemy = CombatComponent->FocusedEnemy->GetActorLocation() - GetOwningActor()->GetActorLocation();


	return RotationMode.Enemy() &&
		CharacterInformation.ViewMode == EALSViewMode::ThirdPerson &&
		CombatComponent;
}
...
void UALSCharacterAnimInstance::UpdateLayerValues()
{
	...
	Legs_Add = GetCurveValue(NAME_Layering_Legs_Add);		//
	Pelvis_add = GetCurveValue(NAME_Layering_Pelvis_Add); //
	...
	//UCPC_Combat* CombatComponent = Character->FindComponentByClass<UCPC_Combat>();
	if (CombatComponent)
	{
		//MyActiveState = CombatComponent->ActiveState;
		bCombatMode_Q = CombatComponent->CombatMode;
		bRight_Q = CombatComponent->bBlock;
		bIsDodge_Q = CombatComponent->bIsDodge;
	}

	//velocity

	/*Dir = CalculateDirection(Speed.Velocity, Speed.CharacterActorRotation);
	FString FloatValueString = FString::Printf(TEXT("%.2f"), Speed.Speed);
	GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, FloatValueString);*/
...
void UALSCharacterAnimInstance::RotateToEnemyCheck(float DeltaSeconds)
{
	if (CombatComponent->FocusedEnemy)
	{

		FVector DirectionToEnemy = CombatComponent->FocusedEnemy->GetActorLocation() - GetOwningActor()->GetActorLocation();
		DirectionToEnemy.Normalize();
		FRotator RotationToEnemy = FRotationMatrix::MakeFromX(DirectionToEnemy).Rotator();
		...
		// Step 2: Check if the Elapsed Delay time exceeds the set delay (mapped to the turn angle range). If so, trigger a Turn In Place.
		if (true/*TurnInPlaceValues.ElapsedDelayTime > ClampedAimAngle*/)
		{

			FRotator TurnInPlaceYawRot = RotationToEnemy;
			...
			FRotator Delta = TurnInPlaceYawRot - CharacterInformation.CharacterActorRotation;
			Delta.Normalize();
			const float TurnAngle = Delta.Yaw;
			GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Emerald, TEXT("RTE"));
			// Step 2: Choose Turn Asset based on the Turn Angle and Stance
			FALSTurnInPlaceAsset TargetTurnAsset;
			if (Stance.Standing())
			{
				if (FMath::Abs(TurnAngle) >=0)
				{
					TargetTurnAsset = TurnAngle < 0.0f
						? TurnInPlaceValues.N_TurnIP_L90
						: TurnInPlaceValues.N_TurnIP_R90;
					GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Emerald, TEXT("RTE 90"));
				}
				else
				{
					TargetTurnAsset = TurnAngle < 0.0f
					...
					GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Emerald, TEXT("RTE 180"));
				}
			}
			else
			{
			...
			if (!false && IsPlayingSlotAnimation(TargetTurnAsset.Animation, TargetTurnAsset.SlotName))
			{
				return;
			}
			PlaySlotAnimationAsDynamicMontage(TargetTurnAsset.Animation, TargetTurnAsset.SlotName, 0.2f, 0.2f,
				TargetTurnAsset.PlayRate * 1, 1, 0.0f, 0);
			...
			{
				Grounded.RotationScale = (TurnAngle / TargetTurnAsset.AnimatedAngle) * TargetTurnAsset.PlayRate * 1;
			}
			else
			{
				Grounded.RotationScale = TargetTurnAsset.PlayRate * 1;
			}
		}
	}
	else
	{
		// No focused enemy, reset rotation related flags
		Grounded.bRotateL = false;
		Grounded.bRotateR = false;
		Grounded.RotateRate = 0.0f;
	}
}

bool UALSCharacterAnimInstance::CanRotateToEnemy() const
{
	//GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Red, TEXT("Not"));
	return CombatComponent->FocusedEnemy != nullptr && CombatComponent->CombatMode;
}

bool UALSCharacterAnimInstance::IsFacingEnemy()	const
{
	if (CombatComponent->FocusedEnemy)
	{
		...
		float TargetYaw = FMath::RadiansToDegrees(FMath::Atan2(DirectionToEnemy.Y, DirectionToEnemy.X));

		// Check if the character is facing the enemy within a certain tolerance
		return FMath::Abs(AimingValues.AimingAngle.X - TargetYaw) < 50.0f;
	}
	return false;
}
...
void UALSCharacterAnimInstance::TurnToEnemy(AActor* FocusedEnemy)
{
	// Step 1: Check if there is a valid focused enemy.
	if (!FocusedEnemy || !FocusedEnemy->IsValidLowLevel())
	{
		// If there is no focused enemy, reset the rotation rate and exit the function.
		Grounded.RotateRate = 0.0f;
		return;
	}
	GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Emerald, TEXT("TENE"));
	// Step 2: Calculate the direction vector to the enemy.
	FVector DirectionToEnemy = FocusedEnemy->GetActorLocation() - GetOwningActor()->GetActorLocation();
	DirectionToEnemy.Normalize();

	// Calculate the yaw from the direction vector.
	float TargetYaw = FMath::RadiansToDegrees(FMath::Atan2(DirectionToEnemy.Y, DirectionToEnemy.X));

	// Step 3: Calculate the desired rotation rate towards the enemy.
	Grounded.RotateRate = FMath::GetMappedRangeValueClamped<float, float>(
		{ RotateInPlace.AimYawRateMinRange, RotateInPlace.AimYawRateMaxRange },
		{ RotateInPlace.MinPlayRate, RotateInPlace.MaxPlayRate },
		FMath::Abs(TargetYaw));
