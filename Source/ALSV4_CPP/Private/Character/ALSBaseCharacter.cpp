...
void AALSBaseCharacter::SetupInputComponent(UInputComponent* PlayerInputComponent)
{
...
void AALSBaseCharacter::ForwardMovementAction_Implementation(float Value)
{
	if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir)
	{
		...
		if (CombatComponent)
		{
			CombatComponent->ForwardDir = Value;
		}
	}
}
...
void AALSBaseCharacter::RightMovementAction_Implementation(float Value)
{
	if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir)
	{
		...
		if (CombatComponent)
		{
			CombatComponent->RightDir = Value;
		}
	}
}
...
void AALSBaseCharacter::JumpAction_Implementation(bool bValue)
{
	if (bValue)
	{
		...
		if (UCPC_Combat* CombatComponent = FindComponentByClass<UCPC_Combat>(); CombatComponent->CombatMode)
		{
			GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Green, TEXT("Dodge"));
...
void AALSBaseCharacter::AimAction_Implementation(bool bValue)
{
	...
	if (bValue)
	{
		...
		if (CombatComponent->CombatMode)
		{
			...
			CombatComponent->Sword->SetCollisionResponseToChannel(ECollisionChannel::ECC_WorldDynamic, ECollisionResponse::ECR_Block);
			GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Green, TEXT("Block On"));
			return;
		}
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Green, TEXT("Block Off"));
		CombatComponent->bBlock = false;
		if (CombatComponent && CombatComponent->Sword)
		{
			CombatComponent->Sword->SetCollisionResponseToChannel(ECollisionChannel::ECC_WorldDynamic, ECollisionResponse::ECR_Ignore);
		}
		if (ViewMode == EALSViewMode::ThirdPerson)
		{
			SetRotationMode(DesiredRotationMode);
...
void AALSBaseCharacter::AttackAction_Implementation(bool bValue)
{
	...
	if (CombatComponent)
	{
		...
		CombatComponent->ActionInput(EAttackMove::LightAttack, OverlayState);
		SetRotationMode(EALSRotationMode::LookingDirection);
	...
	else
	{
		...
		UE_LOG(LogTemp, Error, TEXT("UCPC_Combat component not found"));
	}
